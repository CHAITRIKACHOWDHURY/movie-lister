...
test('check count of children component', () => {
  const fakeMethod = jest.fn();
  render(
    <MovieList
      results={results}
      isLoading={false}
      movieSelected={fakeMethod}/>
  );
  expect(screen.getAllByTestId('Movie').length).toEqual(6);
});
test('check count of children component when loading', () => {
  const fakeMethod = jest.fn();
  render(
    <MovieList
      results={[]}
  ...
  expect(screen.queryAllByTestId('Movie').length).toEqual(0);
});