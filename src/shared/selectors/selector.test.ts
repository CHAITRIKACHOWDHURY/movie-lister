...
import { convertToRoman, getMovieList, getSearcheddMovieList} from "./selector";
...
test('convert number to roman', () => {
    expect(convertToRoman(0)).toEqual('');
    expect(convertToRoman(5)).toEqual('V');
    expect(convertToRoman(19)).toEqual('XIX');
    expect(convertToRoman(1)).toEqual('I');
    expect(convertToRoman(3)).toEqual('III');
    expect(convertToRoman(4)).toEqual('IV');
    expect(convertToRoman(40)).toEqual('XL');
});
test('getMovieList', () => {
    const actual = getMovieList(state);
    expect(actual[0].title).toEqual('Episode IV - A New Hope');
});
test('getSearcheddMovieList', () => {
    const actual = getSearcheddMovieList.resultFunc(getMovieList(state), 'The');
